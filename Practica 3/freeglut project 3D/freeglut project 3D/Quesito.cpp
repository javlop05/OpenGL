...
Quesito::Quesito(int nDivisiones, GLfloat radio, GLfloat altura, TAfin* mT, Color* c, GLuint* tx, int numTx) : Malla(numVertices, vertice, numNormales, normal, numCaras, cara, false, false, mT, c, tx, numTx){
	...
	this->numVertices = nRotaciones*ptsPerfil + 4;
	...
	for (int i = 1; i < this->nDivisiones; i++){ //generar el perfil i-ésimo
		...
		for (int j = 0; j< this->ptsPerfil; j++) {
			...
			PV3D* p = new PV3D((GLfloat)x, perfil[j]->getY(), (GLfloat)z, 1);
			vertice[indice] = p;
		} //for
	} //for
